...
author: "Benjamin Lykke Christensen"
date: "20/3/2026"
...
## Plot: 1952
```{r plot_1952}
ggplot(subset(gapminder, year == 1952),
       aes(gdpPercap, lifeExp, size = pop/100000)) +
...
## Animation (transition_states)
```{r anim_states}
anim <- ggplot(gapminder,
               aes(gdpPercap, lifeExp, size = pop/1000000)) +
  geom_point(aes(color = continent)) +
  scale_x_log10() +
  labs(
...
There are great differences in population sizes, so it makes sense to make a logoritmic scale in order for better comparisons.
2. *Answer: In Figure 1: Who is the outlier (the richest country in 1952) far right on the x axis?* 
The richest counrtry is in Asia
