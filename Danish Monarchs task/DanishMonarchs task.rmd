...
Since we were told that Read_Csv is a tidyverse function, that is the case. 


Read data with it below into a `kings` object
2. What is the result of running `class()` on the `kings` object created with a tidyverse function. 
The result is: > class(kings) [1] "spec_tbl_df" "tbl_df"      "tbl"         "data.frame"

3. How many columns does the object have when created with these different functions?
Read CSV1 gives me 1 
Read CSV_ gives me 1 column
ReadCSV2 gives me 5 columns


4. Show the dataset so that we can see how R interprets each column
The #head function shows the data in a tidy way.

```{r eval = TRUE}
# COMPLETE THE BLANKS BELOW WITH YOUR CODE, then turn the 'eval' flag in this chunk to TRUE.

kings <- read_csv2("data/Kings.csv", na="NULL")
kings
...
* Do your start and end of reign columns contain NAs? Choose the right strategy to deal with 
them: `na.omit()`, `na.rm=TRUE`, `!is.na()`
